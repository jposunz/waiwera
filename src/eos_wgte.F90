...
#include <petsc/finclude/petscsys.h>
  ...
  type, public, extends(eos_type) :: eos_wgte_type
   ...
   contains
     private
     procedure, public :: init => eos_wgte_init
     procedure, public :: destroy => eos_wgte_destroy
     procedure, public :: transition => eos_wgte_transition
     procedure, public :: transition_to_single_phase => eos_wgte_transition_to_single_phase
     procedure, public :: transition_to_two_phase => eos_wgte_transition_to_two_phase
     procedure, public :: bulk_properties => eos_wgte_bulk_properties
     procedure, public :: phase_properties => eos_wgte_phase_properties
     procedure, public :: primary_variables => eos_wgte_primary_variables
     procedure, public :: phase_saturations => eos_wgte_phase_saturations
     procedure, public :: check_primary_variables => eos_wgte_check_primary_variables
  end type eos_wgte_type

contains

!------------------------------------------------------------------------

  subroutine eos_wgte_init(self, json, thermo, logfile)
    !! Initialise pure water, non-condensible gas, tracer and energy EOS.
    ...
    class(eos_wgte_type), intent(in out) :: self
    ...
    case (TYPE_STRING, TYPE_NULL)
       self%scale => eos_wgte_scale_adaptive
       self%unscale => eos_wgte_unscale_adaptive
    ...
    allocate(self%primary_scale(4, 4))
    self%primary_scale = reshape([ &
          ...
          pressure_scale, 1._dp, partial_pressure_scale, liquid_mass_fraction_scale], [4, 4])
...
!------------------------------------------------------------------------
  ...
  end subroutine eos_wgte_destroy

!------------------------------------------------------------------------

  subroutine eos_wgte_transition_to_single_phase(self, old_primary, old_fluid, &
       new_region, primary, fluid, transition, err)
    !! For eos_wgte, make transition from two-phase to single-phase with
    !! specified region.
    ...
    associate (pressure => primary(1), temperature => primary(2), &
         partial_pressure => primary(3), tracer_liquid_mass_fraction => primary(4))
      ...
      tracer_liquid_mass_fraction = min(tracer_liquid_mass_fraction, tracer_liquid_mass_fraction_bound)

    end associate

  end subroutine eos_wgte_transition_to_single_phase

!------------------------------------------------------------------------

  subroutine eos_wgte_transition_to_two_phase(self, saturation_pressure, &
       old_primary, old_fluid, primary, fluid, transition, err)
    !! For eos_wgte, make transition from single-phase to two-phase.
  ...
  end subroutine eos_wgte_transition_to_two_phase

!------------------------------------------------------------------------

  subroutine eos_wgte_transition(self, old_primary, primary, &
       old_fluid, fluid, transition, err)
    !! Check primary variables for eos_wgte and make thermodynamic
  ...
  end subroutine eos_wgte_transition

!------------------------------------------------------------------------

  subroutine eos_wgte_bulk_properties(self, primary, fluid, err)
  ...
  end subroutine eos_wgte_bulk_properties

!------------------------------------------------------------------------

  subroutine eos_wgte_phase_saturations(self, primary, fluid)
  ...
  end subroutine eos_wgte_phase_saturations

!------------------------------------------------------------------------

  subroutine eos_wgte_phase_properties(self, primary, rock, fluid, err)
                     ...
                     associate(water_density => water_properties(1), &
                       ...
                       if (err == 0) then
                          ...
                          if (err == 0) then
                             ...
                             phase%mass_fraction = [(1._dp - xg) * (1._dp - xt), xg * (1._dp - xt), xt]
                             ...
                             !write(*,*) 'phase%mass_fraction = ',phase%mass_fraction
                          else
                             exit
                          end if
                       else
  ...
  end subroutine eos_wgte_phase_properties

!------------------------------------------------------------------------

  subroutine eos_wgte_primary_variables(self, fluid, primary)
    !! Determine primary variables from fluid properties.

    use fluid_module, only: fluid_type

    class(eos_wgte_type), intent(in) :: self
  ...
  end subroutine eos_wgte_primary_variables

!------------------------------------------------------------------------

  subroutine eos_wgte_check_primary_variables(self, fluid, &
       primary, changed, err)
    ...
    associate (total_pressure => primary(1), partial_pressure => primary(3), &
        tracer_mass_fraction => primary(4))
      ...
      if (total_pressure > 0._dp) then
         ...
         if (tracer_mass_fraction > 1._dp) then
              tracer_mass_fraction = 1._dp
              changed = PETSC_TRUE
           else if (tracer_mass_fraction < 0._dp) then
              tracer_mass_fraction = 0._dp
              changed = PETSC_TRUE
           end if
  ...
  end subroutine eos_wgte_check_primary_variables

!------------------------------------------------------------------------

  function eos_wgte_scale_adaptive(self, primary, region) result(scaled_primary)
    !! Non-dimensionalise eos_wgte primary variables by scaling. The
    ...
    scaled_primary(4) = primary(4) / self%primary_scale(4, region)

  end function eos_wgte_scale_adaptive

!------------------------------------------------------------------------

  function eos_wgte_unscale_adaptive(self, scaled_primary, region) result(primary)
    !! Re-dimensionalise eos_wgte scaled primary variables.
    ...
    primary(4) = scaled_primary(4) * self%primary_scale(4, region)

  end function eos_wgte_unscale_adaptive

!------------------------------------------------------------------------

  PetscReal function eos_wgte_saturation_difference(x, context) result(dp)
  ...
  end function eos_wgte_saturation_difference

!------------------------------------------------------------------------

end module eos_wgte_module
