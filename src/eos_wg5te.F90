module eos_wg5te_module
  !! Equation of state for non-isothermal water, non-condensible gas and tracer.
...
#include <petsc/finclude/petscsys.h>
  ...
  type, public, extends(eos_type) :: eos_wg5te_type
     !! Pure water, non-condensible gas, tracer and energy equation of state type.
     private
   ...
   contains
     private
     procedure, public :: init => eos_wg5te_init
     procedure, public :: destroy => eos_wg5te_destroy
     procedure, public :: transition => eos_wg5te_transition
     procedure, public :: transition_to_single_phase => eos_wg5te_transition_to_single_phase
     procedure, public :: transition_to_two_phase => eos_wg5te_transition_to_two_phase
     procedure, public :: bulk_properties => eos_wg5te_bulk_properties
     procedure, public :: phase_properties => eos_wg5te_phase_properties
     procedure, public :: primary_variables => eos_wg5te_primary_variables
     procedure, public :: phase_saturations => eos_wg5te_phase_saturations
     procedure, public :: check_primary_variables => eos_wg5te_check_primary_variables
  end type eos_wg5te_type

contains

!------------------------------------------------------------------------

  subroutine eos_wg5te_init(self, json, thermo, logfile)
    !! Initialise pure water, non-condensible gas, 5 tracers and energy EOS.
    ...
    class(eos_wg5te_type), intent(in out) :: self
    ...
    PetscReal :: pressure_scale, temperature_scale, partial_pressure_scale, liquid_mass_fraction_scale
    PetscInt :: scale_type
    PetscErrorCode :: err
    ...
    PetscReal, parameter :: default_liquid_tracer_mass_fraction = 0._dp
    ...
    PetscReal, parameter :: default_liquid_mass_fraction_scale = 1.0e-6_dp !! Default scale factor for non-dimensionalising mass fraction

    self%name = "wg5te"
    self%description = "Water, non-condensible gas, 5 tracers and energy"
    self%primary_variable_names = [ &
         "pressure                     ", &
         "temperature/vapour_saturation", &
         "gas partial pressure         ", &
         "tracer1 liquid mass fraction ", &
         "tracer2 liquid mass fraction ", &
         "tracer3 liquid mass fraction ", &
         "tracer4 liquid mass fraction ", &
         "tracer5 liquid mass fraction "]

    self%num_primary_variables = size(self%primary_variable_names)
    self%num_phases = 2
    self%phase_names = ["liquid", "vapour"]
    self%num_components = 7
    self%component_names = ["water  ", "gas    ", "tracer1", "tracer2", "tracer3", "tracer4", "tracer5"]

    self%default_primary = [default_pressure, default_temperature, &
         default_gas_partial_pressure, default_liquid_tracer_mass_fraction, &
         default_liquid_tracer_mass_fraction, default_liquid_tracer_mass_fraction, &
         default_liquid_tracer_mass_fraction, default_liquid_tracer_mass_fraction]
    ...
    call fson_get_mpi(json, "eos.primary.scale.liquid_mass_fraction", default_liquid_mass_fraction_scale, &
         liquid_mass_fraction_scale, logfile)

    scale_type = fson_type_mpi(json, "eos.primary.scale.partial_pressure")
    select case (scale_type)
    case (TYPE_STRING, TYPE_NULL)
       self%scale => eos_wg5te_scale_adaptive
       self%unscale => eos_wg5te_unscale_adaptive
    ...
    allocate(self%primary_scale(8, 4))
    self%primary_scale = reshape([ &
          pressure_scale, temperature_scale, partial_pressure_scale, liquid_mass_fraction_scale, &
          liquid_mass_fraction_scale, liquid_mass_fraction_scale, liquid_mass_fraction_scale, &
          liquid_mass_fraction_scale, pressure_scale, temperature_scale, partial_pressure_scale, &
          liquid_mass_fraction_scale, liquid_mass_fraction_scale, liquid_mass_fraction_scale, &
          liquid_mass_fraction_scale, liquid_mass_fraction_scale, &
          0._dp, 0._dp, 0._dp, 0._dp, 0._dp, 0._dp, 0._dp, 0._dp, &
          pressure_scale, 1._dp, partial_pressure_scale, liquid_mass_fraction_scale, &
          liquid_mass_fraction_scale, liquid_mass_fraction_scale, liquid_mass_fraction_scale, &
          liquid_mass_fraction_scale], [8, 4])
    ...
    f => eos_wg5te_saturation_difference
    pinterp => self%primary_variable_interpolator
    call self%saturation_line_finder%init(f, context = pinterp)

  end subroutine eos_wg5te_init

!------------------------------------------------------------------------

  subroutine eos_wg5te_destroy(self)
    !! Destroy pure water, non-condensible gas, tracer and energy EOS.
  ...
  end subroutine eos_wg5te_destroy

!------------------------------------------------------------------------

  subroutine eos_wg5te_transition_to_single_phase(self, old_primary, old_fluid, &
       new_region, primary, fluid, transition, err)
    !! For eos_wg5te, make transition from two-phase to single-phase with
    !! specified region.
    ...
    PetscReal :: tracer_liquid_mass_fraction_bound
    PetscReal, parameter :: small = 1.e-6_dp

    err = 0
    transition = PETSC_FALSE

    !write(*,*) 'pre-primary = ', primary
    if (new_region == 1) then
       saturation_bound = 0._dp
       pressure_factor = 1._dp + small
       tracer_liquid_mass_fraction_bound = 1._dp
    else
       saturation_bound = 1._dp
       pressure_factor = 1._dp - small
       tracer_liquid_mass_fraction_bound = 0._dp
    end if

    associate (pressure => primary(1), temperature => primary(2), &
         partial_pressure => primary(3), tracer1_liquid_mass_fraction => primary(4), &
         tracer2_liquid_mass_fraction => primary(5), &
         tracer3_liquid_mass_fraction => primary(6), &
         tracer4_liquid_mass_fraction => primary(7), &
         tracer5_liquid_mass_fraction => primary(8) )
      ...
      tracer1_liquid_mass_fraction = min(tracer1_liquid_mass_fraction, tracer_liquid_mass_fraction_bound)
      tracer2_liquid_mass_fraction = min(tracer2_liquid_mass_fraction, tracer_liquid_mass_fraction_bound)
      tracer3_liquid_mass_fraction = min(tracer3_liquid_mass_fraction, tracer_liquid_mass_fraction_bound)
      tracer4_liquid_mass_fraction = min(tracer4_liquid_mass_fraction, tracer_liquid_mass_fraction_bound)
      tracer5_liquid_mass_fraction = min(tracer5_liquid_mass_fraction, tracer_liquid_mass_fraction_bound)

    end associate
    !write(*,*) 'post-primary = ', primary

  end subroutine eos_wg5te_transition_to_single_phase

!------------------------------------------------------------------------

  subroutine eos_wg5te_transition_to_two_phase(self, saturation_pressure, &
       old_primary, old_fluid, primary, fluid, transition, err)
    !! For eos_wg5te, make transition from single-phase to two-phase.
  ...
  end subroutine eos_wg5te_transition_to_two_phase

!------------------------------------------------------------------------

  subroutine eos_wg5te_transition(self, old_primary, primary, &
       old_fluid, fluid, transition, err)
    !! Check primary variables for eos_wg5te and make thermodynamic
  ...
  end subroutine eos_wg5te_transition

!------------------------------------------------------------------------

  subroutine eos_wg5te_bulk_properties(self, primary, fluid, err)
    ...
    ! if (primary(4) > 1e-8) then
        ! write(*,*) 'here bulk primary = ',primary
    ! else
        ! if (primary(5) > 1e-8) then
            ! write(*,*) 'here bulk primary = ',primary
        ! else
            ! if (primary(6) > 1e-8) then
                ! write(*,*) 'here bulk primary = ',primary
            ! else
                ! if (primary(7) > 1e-8) then
                    ! write(*,*) 'here bulk primary = ',primary
                ! else
                    ! write(*,*) '     bulk primary = ',primary   
                ! end if
            ! end if
        ! end if
    ! end if
    ...
    associate(partial_pressure => primary(3))
      ...
      fluid%partial_pressure(3) = 0._dp
      fluid%partial_pressure(4) = 0._dp
      fluid%partial_pressure(5) = 0._dp
      fluid%partial_pressure(6) = 0._dp
      fluid%partial_pressure(7) = 0._dp
    end associate
  ...
  end subroutine eos_wg5te_bulk_properties

!------------------------------------------------------------------------

  subroutine eos_wg5te_phase_saturations(self, primary, fluid)
  ...
  end subroutine eos_wg5te_phase_saturations

!------------------------------------------------------------------------

  subroutine eos_wg5te_phase_properties(self, primary, rock, fluid, err)
    ...
    ! Locals:
    PetscInt :: p, phases
    PetscReal :: sl, xg, xt1, xt2, xt3, xt4, xt5
    ...
    ! if (primary(4) > 1e-8) then
        ! write(*,*) 'here phas primary = ',primary
    ! else
        ! if (primary(5) > 1e-8) then
            ! write(*,*) 'here phas primary = ',primary
        ! else
               ...
               if (btest(phases, p - 1)) then
                  ...
                  if (err == 0) then
                     ...
                     associate(water_density => water_properties(1), &
                       ...
                       ! if ((xt1 + xt2 + xt3 + xt4) > 0._dp) then
                          ! write(*,*) 'primary check = ', primary
                       ! end if
                       ...
                       if (err == 0) then
                          ...
                          if (err == 0) then
                             ...
                             phase%mass_fraction = [(1._dp - (xg + xt1 + xt2 + xt3 + xt4 + xt5)), &
                             xg , xt1, xt2, xt3, xt4, xt5]
    ...
    ! if (phase%mass_fraction(3) > 1e-8) then
        ! write(*,*) 'here phas mass_fr = ',phase%mass_fraction
    ! else
        ! if (phase%mass_fraction(4) > 1e-8) then
            ! write(*,*) 'here phas mass_fr = ',phase%mass_fraction
        ! else
            ! if (phase%mass_fraction(5) > 1e-8) then
                ! write(*,*) 'here phas mass_fr = ',phase%mass_fraction
            ! else
                ! if (phase%mass_fraction(6) > 1e-8) then
                    ! write(*,*) 'here phas mass_fr = ',phase%mass_fraction
                ! else
                    ! write(*,*) '     phas mass_fr = ',phase%mass_fraction   
                ! end if
            ! end if
        ! end if
    ! end if
  ...
  end subroutine eos_wg5te_phase_properties

!------------------------------------------------------------------------

  subroutine eos_wg5te_primary_variables(self, fluid, primary)
    !! Determine primary variables from fluid properties.

    use fluid_module, only: fluid_type

    class(eos_wg5te_type), intent(in) :: self
    ...
    primary(4) = fluid%phase(1)%mass_fraction(3)
    primary(5) = fluid%phase(1)%mass_fraction(4)
    primary(6) = fluid%phase(1)%mass_fraction(5)
    primary(7) = fluid%phase(1)%mass_fraction(6)
    primary(8) = fluid%phase(1)%mass_fraction(7)
    
    ! if (primary(4) > 1e-8) then
        ! write(*,*) 'here load primary = ',primary
    ! else
        ! if (primary(5) > 1e-8) then
            ! write(*,*) 'here load primary = ',primary
        ! else
  ...
  end subroutine eos_wg5te_primary_variables

!------------------------------------------------------------------------

  subroutine eos_wg5te_check_primary_variables(self, fluid, &
       primary, changed, err)
    ...
    associate (total_pressure => primary(1), partial_pressure => primary(3), &
        tracer1_mass_fraction => primary(4), tracer2_mass_fraction => primary(5), &
        tracer3_mass_fraction => primary(6), tracer4_mass_fraction => primary(7), &
        tracer5_mass_fraction => primary(8))
      ...
      if (total_pressure > 0._dp) then
         ...
         if (tracer1_mass_fraction > 1._dp) then
              tracer1_mass_fraction = 1._dp
              changed = PETSC_TRUE
           else if (tracer1_mass_fraction < 0._dp) then
              tracer1_mass_fraction = 0._dp
              changed = PETSC_TRUE
           end if

         if (tracer2_mass_fraction > 1._dp) then
              tracer2_mass_fraction = 1._dp
              changed = PETSC_TRUE
           else if (tracer2_mass_fraction < 0._dp) then
              tracer2_mass_fraction = 0._dp
              changed = PETSC_TRUE
           end if

         if (tracer3_mass_fraction > 1._dp) then
              tracer3_mass_fraction = 1._dp
              changed = PETSC_TRUE
           else if (tracer3_mass_fraction < 0._dp) then
              tracer3_mass_fraction = 0._dp
              changed = PETSC_TRUE
           end if

         if (tracer4_mass_fraction > 1._dp) then
              tracer4_mass_fraction = 1._dp
              changed = PETSC_TRUE
           else if (tracer4_mass_fraction < 0._dp) then
              tracer4_mass_fraction = 0._dp
              changed = PETSC_TRUE
           end if

         if (tracer5_mass_fraction > 1._dp) then
              tracer5_mass_fraction = 1._dp
              changed = PETSC_TRUE
           else if (tracer5_mass_fraction < 0._dp) then
              tracer5_mass_fraction = 0._dp
              changed = PETSC_TRUE
           end if
  ...
  end subroutine eos_wg5te_check_primary_variables

!------------------------------------------------------------------------

  function eos_wg5te_scale_adaptive(self, primary, region) result(scaled_primary)
    !! Non-dimensionalise eos_wg5te primary variables by scaling. The
    ...
    scaled_primary(4:8) = primary(4:8) / self%primary_scale(4:8, region)

  end function eos_wg5te_scale_adaptive

!------------------------------------------------------------------------

  function eos_wg5te_unscale_adaptive(self, scaled_primary, region) result(primary)
    !! Re-dimensionalise eos_wg5te scaled primary variables.
    ...
    primary(4:8) = scaled_primary(4:8) * self%primary_scale(4:8, region)

  end function eos_wg5te_unscale_adaptive

!------------------------------------------------------------------------

  PetscReal function eos_wg5te_saturation_difference(x, context) result(dp)
  ...
  end function eos_wg5te_saturation_difference

!------------------------------------------------------------------------

end module eos_wg5te_module
