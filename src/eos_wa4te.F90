module eos_wa4te_module
  !! Equation of state for non-isothermal water and air NCG.

#include <petsc/finclude/petscsys.h>

  use petscsys
  use eos_wg4te_module
  use ncg_air_thermodynamics_module

  implicit none
  private

  type, public, extends(eos_wg4te_type) :: eos_wa4te_type
     !! Equation of state object for non-isothermal water and air NCG.
   contains
     private
     procedure, public :: init => eos_wa4te_init
     procedure, public :: destroy => eos_wa4te_destroy
  end type eos_wa4te_type

contains

!------------------------------------------------------------------------

  subroutine eos_wa4te_init(self, json, thermo, logfile)
    !! Initialise eos_wa4te object.
    ...
    class(eos_wa4te_type), intent(in out) :: self
    ...
    call self%eos_wg4te_type%init(json, thermo, logfile)

    self%name = "wa4te"
    self%description = "Water, air NCG, 4 tracers and energy"
  ...
  end subroutine eos_wa4te_init

!------------------------------------------------------------------------

  subroutine eos_wa4te_destroy(self)
    !! Destroy eos_wa4te object.

    class(eos_wa4te_type), intent(in out) :: self

    deallocate(self%gas)
    call self%eos_wg4te_type%destroy()

  end subroutine eos_wa4te_destroy

!------------------------------------------------------------------------

end module eos_wa4te_module
