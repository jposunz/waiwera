...
#include <petsc/finclude/petscsys.h>
  ...
  type, public, extends(eos_type) :: eos_wg4te_type
   ...
   contains
     private
     procedure, public :: init => eos_wg4te_init
     procedure, public :: destroy => eos_wg4te_destroy
     procedure, public :: transition => eos_wg4te_transition
     procedure, public :: transition_to_single_phase => eos_wg4te_transition_to_single_phase
     procedure, public :: transition_to_two_phase => eos_wg4te_transition_to_two_phase
     procedure, public :: bulk_properties => eos_wg4te_bulk_properties
     procedure, public :: phase_properties => eos_wg4te_phase_properties
     procedure, public :: primary_variables => eos_wg4te_primary_variables
     procedure, public :: phase_saturations => eos_wg4te_phase_saturations
     procedure, public :: check_primary_variables => eos_wg4te_check_primary_variables
  end type eos_wg4te_type

contains

!------------------------------------------------------------------------

  subroutine eos_wg4te_init(self, json, thermo, logfile)
    !! Initialise pure water, non-condensible gas, 4 tracers and energy EOS.
    ...
    class(eos_wg4te_type), intent(in out) :: self
    ...
    self%name = "wg4te"
    self%description = "Water, non-condensible gas, 4 tracers and energy"
    ...
    self%component_names = ["water  ", "gas    ", "tracer1", "tracer2", "tracer3", "tracer4"]
    ...
    self%default_primary = [default_pressure, default_temperature, &
         ...
         default_liquid_tracer_mass_fraction]
    ...
    case (TYPE_STRING, TYPE_NULL)
       self%scale => eos_wg4te_scale_adaptive
       self%unscale => eos_wg4te_unscale_adaptive
    ...
    allocate(self%primary_scale(7, 4))
    self%primary_scale = reshape([ &
          ...
          pressure_scale, temperature_scale, partial_pressure_scale, liquid_mass_fraction_scale, &
          liquid_mass_fraction_scale, liquid_mass_fraction_scale, liquid_mass_fraction_scale, &
          0._dp, 0._dp, 0._dp, 0._dp, 0._dp, 0._dp, 0._dp, &
          pressure_scale, 1._dp, partial_pressure_scale, liquid_mass_fraction_scale, &
          liquid_mass_fraction_scale, liquid_mass_fraction_scale, liquid_mass_fraction_scale], [7, 4])
...
!------------------------------------------------------------------------
  ...
  end subroutine eos_wg4te_destroy

!------------------------------------------------------------------------

  subroutine eos_wg4te_transition_to_single_phase(self, old_primary, old_fluid, &
       new_region, primary, fluid, transition, err)
    !! For eos_wg4te, make transition from two-phase to single-phase with
    !! specified region.
  ...
  end subroutine eos_wg4te_transition_to_single_phase

!------------------------------------------------------------------------

  subroutine eos_wg4te_transition_to_two_phase(self, saturation_pressure, &
       old_primary, old_fluid, primary, fluid, transition, err)
    !! For eos_wg4te, make transition from single-phase to two-phase.
  ...
  end subroutine eos_wg4te_transition_to_two_phase

!------------------------------------------------------------------------

  subroutine eos_wg4te_transition(self, old_primary, primary, &
       old_fluid, fluid, transition, err)
    !! Check primary variables for eos_wg4te and make thermodynamic
  ...
  end subroutine eos_wg4te_transition

!------------------------------------------------------------------------

  subroutine eos_wg4te_bulk_properties(self, primary, fluid, err)
  ...
  end subroutine eos_wg4te_bulk_properties

!------------------------------------------------------------------------

  subroutine eos_wg4te_phase_saturations(self, primary, fluid)
  ...
  end subroutine eos_wg4te_phase_saturations

!------------------------------------------------------------------------

  subroutine eos_wg4te_phase_properties(self, primary, rock, fluid, err)
    ...
    ! Locals:
    PetscInt :: p, phases
    PetscReal :: sl, xg, xt1, xt2, xt3, xt4
                     ...
                     associate(water_density => water_properties(1), &
                       ...
                       if (err == 0) then
                          ...
                          if (err == 0) then
                             ...
                             phase%mass_fraction = [(1._dp - (xg + xt1 + xt2 + xt3 + xt4)), &
                             xg , xt1, xt2, xt3, xt4]
  ...
  end subroutine eos_wg4te_phase_properties

!------------------------------------------------------------------------

  subroutine eos_wg4te_primary_variables(self, fluid, primary)
    !! Determine primary variables from fluid properties.

    use fluid_module, only: fluid_type

    class(eos_wg4te_type), intent(in) :: self
  ...
  end subroutine eos_wg4te_primary_variables

!------------------------------------------------------------------------

  subroutine eos_wg4te_check_primary_variables(self, fluid, &
       primary, changed, err)
  ...
  end subroutine eos_wg4te_check_primary_variables

!------------------------------------------------------------------------

  function eos_wg4te_scale_adaptive(self, primary, region) result(scaled_primary)
    !! Non-dimensionalise eos_wg4te primary variables by scaling. The
    ...
    scaled_primary(4:7) = primary(4:7) / self%primary_scale(4:7, region)

  end function eos_wg4te_scale_adaptive

!------------------------------------------------------------------------

  function eos_wg4te_unscale_adaptive(self, scaled_primary, region) result(primary)
    !! Re-dimensionalise eos_wg4te scaled primary variables.
    ...
    primary(4:7) = scaled_primary(4:7) * self%primary_scale(4:7, region)

  end function eos_wg4te_unscale_adaptive

!------------------------------------------------------------------------

  PetscReal function eos_wg4te_saturation_difference(x, context) result(dp)
  ...
  end function eos_wg4te_saturation_difference

!------------------------------------------------------------------------

end module eos_wg4te_module
